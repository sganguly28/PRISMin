...
nuTree2GlobesFlux::nuTree2GlobesFlux(TTree *tree) : fChain(0)
{
  ...
  // used to generate this class and read the Tree.
  if (tree == 0) {
    TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("all_flux_laura_v3r5p4_nu_dec2422.root");
    if (!f || !f->IsOpen()) {
      f = new TFile("all_flux_laura_v3r5p4_nu_dec2422.root");
    }
    f->GetObject("ANNIEOutTree",tree);
  }
  Init(tree);
...
void nuTree2GlobesFlux::Init(TTree *tree)
{
  ...
  TFile* tbf = new TFile("/dune/app/users/sganguly/newdev_august19/g4lbne/example/531mhz_down10_up20_buff30.root","READ");
  //TFile* tbf = new TFile("/dune/app/users/sganguly/newdev_august19/g4lbne/example/531mhz_fnal-real_buffertime_down30_up20_buf10_20hz_1e5.root","READ"); 
