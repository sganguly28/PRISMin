...
void nuTree2GlobesFlux::Loop()
{
  ...
  const int nEbins=7; // initially set at 7
  ...
  // make the histograms for the different flux b
  ...
  TH1D* BeamEspect_numu = new TH1D("BeamEspect_numu","BeamEspect_numu",nhistbins,lowE,hiE);
  TH1D* BeamEspect_under = new TH1D("BeamEspect_under","BeamEspect_under",nhistbins,lowE,hiE);
  TH1D* BeamEspect_vmu_under = new TH1D("BeamEspect_vmu_under","BeamEspect_vmu_under",nhistbins,lowE,hiE);
  TH1D* BeamEspect_vmubar_under = new TH1D("BeamEspect_vmubar_under","BeamEspect_vmubar_under",nhistbins,lowE,hiE);
  TH1D* BeamEspect_ve_under = new TH1D("BeamEspect_ve_under","BeamEspect_ve_under",nhistbins,lowE,hiE);
  TH1D* BeamEspect_vebar_under = new TH1D("BeamEspect_vebar_under","BeamEspect_vebar_under",nhistbins,lowE,hiE);

  TH1D* DT = new TH1D("DT","DT",800,-4.0,4.0);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(nutype==14){
      BeamEspect_numu->Fill(nuE,theweight);
    }
    ...
    if(dTr>(hR-1.88) && dTr<=lR){
      BeamEspect_under->Fill(nuE,theweight);
      if(nutype==14){BeamEspect_vmu_under->Fill(nuE,theweight);}
      if(nutype==12){BeamEspect_ve_under->Fill(nuE,theweight);}
      if(nutype==-14){BeamEspect_vmubar_under->Fill(nuE,theweight);}
      if(nutype==-12){BeamEspect_vebar_under->Fill(nuE,theweight);}
    }
	...
	DT->Fill(dTrlow,theweight);

	//cout<<" dTrlow = "<<dTrlow<<endl;
	//cout<<" dTrhi = "<<dTrhi<<endl;

	if(dTrlow>(hR-1.88) && dTrlow<=lR){
	  //cout<<" dTrlow inside lp1 underbin = "<<dTrlow<<endl;
	  BeamEspect_under->Fill(nuE,theweight);
	  if(nutype==14){BeamEspect_vmu_under->Fill(nuE,theweight);}
	  if(nutype==12){BeamEspect_ve_under->Fill(nuE,theweight);}
	  if(nutype==-14){BeamEspect_vmubar_under->Fill(nuE,theweight);}
	...
	for(int aa=0; aa<nEbins; aa++){
	  ...
	  if( dTrlow>lR && dTrlow<=hR ){
	    if(aa==0 && dTrlow<bc[aa]){
	      //cout<<" dTrlow inside lp1 = "<<dTrlow<<endl;
	    ...
	    if(aa>0 && dTrlow>bc[aa-1] && dTrlow<bc[aa]){
	      ...
	      //cout<<" dTrlow inside lp 2 = "<<dTrlow<<endl;
	  ...
	  if( dTrhi>lR && dTrhi<=hR ){
	    if(aa==0 && dTrhi<bc[aa]){
	      //cout<<" dTrhi inside lp1 = "<<dTrhi<<endl;
	      BeamEspect_bin[aa]->Fill(nuE,theweight);
	      if(nutype==12) BeamEspect_ve_bin[aa]->Fill(nuE,theweight);
	      if(nutype==14) BeamEspect_vmu_bin[aa]->Fill(nuE,theweight);
	      if(nutype==-12) BeamEspect_vebar_bin[aa]->Fill(nuE,theweight);
	    ...
	    if(aa>0 && dTrhi>bc[aa-1] && dTrhi<bc[aa]){
	      //cout<<" dTrhi inside lp2 = "<<dTrhi<<endl;
	      BeamEspect_bin[aa]->Fill(nuE,theweight);
	      if(nutype==12) BeamEspect_ve_bin[aa]->Fill(nuE,theweight);
	      if(nutype==14) BeamEspect_vmu_bin[aa]->Fill(nuE,theweight);
	      if(nutype==-12) BeamEspect_vebar_bin[aa]->Fill(nuE,theweight);
  ...
  TFile* theoutputhists = new TFile("FD_Laura_sliced_flux.root","RECREATE");
  //BeamTiming->Write();

  double POTnorm = (1.0/nPOT);
  BeamEspect->Scale(POTnorm);
  BeamEspect_numu->Scale(POTnorm);
  BeamEspect_under->Scale(POTnorm);
  BeamEspect_vmu_under->Scale(POTnorm);
  BeamEspect_vmubar_under->Scale(POTnorm);
  BeamEspect_ve_under->Scale(POTnorm);
  BeamEspect_vebar_under->Scale(POTnorm);
  BeamEspect->Write();
  BeamEspect_numu->Write();
  DT->Write();

  BeamEspect_under->Write();
  BeamEspect_vmu_under->Write();
  BeamEspect_vmubar_under->Write();
  BeamEspect_ve_under->Write();
  BeamEspect_vebar_under->Write();
  ...
  TString FFileNameUnder;
  if(isFHC) FFileNameUnder+="FHC_";
  else FFileNameUnder+="RHC_";
  if(isNear) FFileNameUnder+="ND_";
  else FFileNameUnder+="FD_";
  FFileNameUnder+="Flux_under";
  FFileNameUnder+=".txt";

  ofstream nufluxUnder;
  nufluxUnder.open(FFileNameUnder);
  for(int j=0; j<nhistbins; j++){
    double theEnergyUnder = BeamEspect_ve_under->GetBinCenter(j+1);
    double theFluxUnder = BeamEspect_ve_under->GetBinContent(j+1);
    nufluxUnder<<theEnergyUnder<<" "<<theFluxUnder<<" ";

    theFluxUnder = BeamEspect_vmu_under->GetBinContent(j+1);
    nufluxUnder<<theFluxUnder<<" 0.0 ";
    theFluxUnder = BeamEspect_vebar_under->GetBinContent(j+1);
    nufluxUnder<<theFluxUnder<<" ";
    theFluxUnder = BeamEspect_vmubar_under->GetBinContent(j+1);
    nufluxUnder<<theFluxUnder<<" 0.0"<<endl;
  }
  nufluxUnder.close();
  ...
  BeamEspect_numu->SetLineColor(kRed);
  BeamEspect_numu->SetLineWidth(2);
  BeamEspect_numu->Draw("hist p L");
  for(int i=0; i<nEbins; i++){
    BeamEspect_vmu_bin[i]->SetLineColor(kBlue+i);
    BeamEspect_vmu_bin[i]->Draw("SAME hist p L");
  }
  BeamEspect_vmu_under->SetLineColor(kGreen);
  BeamEspect_vmu_under->Draw("SAME hist p L");
