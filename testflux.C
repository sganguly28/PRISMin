...
void testflux(){
  ...
  FDFlux = (TH1D*)gDirectory->Get("FD_nu_numu");
  ...
  Enu_all = (TH1D*)gDirectory->Get("BeamEspect_numu");
  ...
  TH1D* toth = StroboFlux_bin0;
  toth->Add(StroboFlux_bin1,1);
  toth->Add(StroboFlux_bin2,1);
  toth->Add(StroboFlux_bin3,1);
  toth->Add(StroboFlux_bin4,1);
  toth->Add(StroboFlux_bin5,1);
  toth->Add(StroboFlux_bin6,1);
  toth->Add(StroboFlux_underbin,1);
  toth->SetLineColor(kRed+1);
  toth->SetLineWidth(3);
  ...
  //gPad->SetLogy();
  toth->Draw("hist p L");
  StroboFlux_bin0->Draw("SAME hist p L");
